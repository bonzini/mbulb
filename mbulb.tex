...
scaling operation followed by a rotation) is more compactly represented
by a quaternion $q$.  \emph{Note that the operation that
  will be performed on the quaternion is \textbf{not} $q\leftarrow q^2+c$,
  so this will still result in a Mandelbulb} rather than the
disappointing quaternion Julia sets.  Only, the 3-tuples of
Section~\ref{sec:2d-3d} will be replaced a well-known mathematical
object, the quaternion.
...
This elegant iteration is the \emph{quaternion formulation of the
  Mandelbrot set}.  However, equation~\ref{eq:coolness} does not generalize
to $n>2$ and, when drawn in 3D it results simply in the ``lathed'' 2D
...
to take a step back and look at equation~\ref{eq:quat} with 3D eyes.
