...
represented by a quaternion $q$.  \emph{Note that the operation that
  will be performed on the quaternion is \textbf{not} $q=q^2+c$, so this
...
\begin{equation*}
\vec{v}=\vec{v}^{n/2} \vec{i} \vec{\bar v}^{n/2}+\vec{c}
\end{equation*}

This is the \emph{quaternion formulation of the Mandelbrot set}; when
drawn in 3D it results in the ``lathed'' Mandelbrot set.  So, for the
sake of generalizing to 3D, we'll have to take a step back and look at
equation~\ref{eq:quat} with 3D eyes.
